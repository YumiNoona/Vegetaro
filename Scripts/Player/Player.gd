...
func enable_momentum():
	if not Global.on_enemy_died.is_connected(_on_momentum_enemy_killed):
		Global.on_enemy_died.connect(_on_momentum_enemy_killed)

func _on_momentum_enemy_killed(_enemy):
	if momentum_stacks < MOMENTUM_MAX_STACKS:
...
func _on_health_component_on_unit_died() -> void:
	if Global.on_enemy_died.is_connected(_on_momentum_enemy_killed):
		Global.on_enemy_died.disconnect(_on_momentum_enemy_killed)
	Global.player = null

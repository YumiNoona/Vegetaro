...
func apply_passive() -> void:
	...
	if heal_on_dash > 0.0:
		if not Global.player.on_dash.is_connected(_on_player_dash):
			Global.player.on_dash.connect(_on_player_dash)
	if double_coins:
		if not Global.on_enemy_died.is_connected(_on_enemy_died):
	...
	if speed_on_kill > 0.0:
		if not Global.on_enemy_died.is_connected(_on_enemy_killed):
			Global.on_enemy_died.connect(_on_enemy_killed)
	if coins_on_dash > 0:
		if not Global.player.on_dash.is_connected(_on_dash_coins):
			Global.player.on_dash.connect(_on_dash_coins)
	if cooldown_reduction > 0.0:
...
func _on_player_hit(hitbox: HitboxComponent):
	if hitbox and hitbox.source and hitbox.source.has_node("HealthComponent"):

...
func setup(boss_instance: Boss, boss_name: String):
	...
	if boss and boss.has_node("HealthComponent"):
		...
		if not health_comp.on_health_changed.is_connected(_on_health_changed):
			health_comp.on_health_changed.connect(_on_health_changed)
		if not health_comp.on_unit_died.is_connected(_on_boss_died):
...
func _exit_tree():
	if is_instance_valid(boss) and boss.has_node("HealthComponent"):
		var health_comp = boss.get_node("HealthComponent")

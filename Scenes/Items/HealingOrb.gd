...
func _on_body_entered(body: Node) -> void:
	if body.is_in_group("player") and body.has_node("HealthComponent"):
		body.get_node("HealthComponent").heal(heal_amount)
		queue_free()
	elif body.is_in_group("boss") and body.has_node("HealthComponent"):
		body.get_node("HealthComponent").heal(heal_amount * 2)
		queue_free()
